...
current_received_pose_L = None
current_received_pose_R = None
...
def update_and_move_pose(robot, received_pose_dict, initial_pose,cycledu,cycle_index):
    """
    ...
    index = received_pose_dict['index']
    ...
    print(f"缩放偏移: [{scaled_pos[0]:.4f}, {scaled_pos[1]:.4f}, {scaled_pos[2]:.4f}] "
          f"数据总延迟: {time_diff_current_odom:.4f}秒, "
          f"函数执行: {func_execution_time:.4f}秒"
          f"发送延迟: {time_diff_send_odom:.4f}秒"
          f"index: {index}"
          f"cindex: {cycle_index}"
          f"cdu: {cycledu:.4f}秒"
          )
    if not is_safe:
...
def network_receiver(sock):
    """接收网络数据的线程函数"""
    global current_received_pose_L
    global current_received_pose_R
                ...
                while "\n" in buffer:
                    ...
                    try:
                        ...
                        with pose_lock:
                            ...
                            camera_id = received_pose_dict['camera_id']
                            #print("received_pose: ", camera_id)
                            if(camera_id=="C1"):
                                current_received_pose_L = received_pose_dict
                            else:
                                current_received_pose_R = received_pose_dict
                            #print("更新当前接收到的pose")
...
def robot_controller(robot, initial_pose,lorr):
    """机器人控制线程函数"""
    global move_enabled,  current_received_pose_L,current_received_pose_R, exit_program
    
    print("机器人控制器已启动")
    
    # 添加移动频率控制
    last_move_time = 0
    move_interval = 0.01  # 100ms移动一次，避免过于频繁
    last_print_time = 0
    ...
    while not exit_program:
        ...
        current_time = time.time()
        cycledu = current_time -last_current_time  
        current_received_pose = None
        with pose_lock:
                if( lorr=="L"):
                    received_pose = current_received_pose_L
                    current_received_pose= current_received_pose_L
                else:
                    received_pose = current_received_pose_R
                    current_received_pose = current_received_pose_R
        # 定期打印位姿对比信息
        ...
        if move_enabled and pause_event.is_set() and (current_time - last_move_time) >= move_interval:
            ...
            if current_received_pose_copy is not None:
                try:
                    ...
                    do_update =True
                    movestart = time.time()
                    current_pose = update_and_move_pose(robot, current_received_pose_copy, initial_pose,cycledu,cycle_index)
                    movedu = time.time() - movestart
                ...
                nocount += 1
        workdu =  time.time() - current_time
        #print("循环周期: %.4f 秒, 工作时间: %.4f 秒 do_update: %s movedu %.4f"%(cycledu,workdu,do_update,movedu))
...
def main():
    global exit_program,gripper_L,gripper_R
    ...
    if(b_robot_L):
        ...
        print("L初始化夹爪...")
        open_gripper(gripper_L)
        state_reader_L = FrankaStateReader(robot_L,gripper_L, "franka_L")
        # 先移动到初始位置（使用平滑移动）
        print("L移动到初始位置...")
        #goto_pose_smooth(robot_L, initial_pose, velocity=0.05, acceleration=0.01, jerk=0.02)
    if(b_robot_R):
        ...
        print("R初始化夹爪...")
        open_gripper(gripper_R)
        state_reader_R = FrankaStateReader(robot_R,gripper_R, "franka_R")
        # 先移动到初始位置（使用平滑移动）
        print("R移动到初始位置...")
        goto_pose_smooth(robot_R, initial_pose, velocity=0.05, acceleration=0.01, jerk=0.02)
...
# 设置初始状态为kaishi
    ...
    try:
        ...
        robot_L_thread = threading.Thread(target=robot_controller, args=(robot_L, initial_pose,"L"), daemon=True)
        robot_R_thread = threading.Thread(target=robot_controller, args=(robot_R, initial_pose,"R"), daemon=True)
