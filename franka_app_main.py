...
class FrankaStateReader:
    def __init__(self, robot, gripper, robot_name="franka", enable_ros_publishing=True):
        """
        初始化状态读取器
        Args:
            robot: Franka机器人实例
        ...
        self.current_gripper_is_grasped = None
        self.current_gripper_max_width = None
        ...
        if self.enable_ros_publishing:
            ...
            self.gripper_width_pub = rospy.Publisher(f'/{robot_name}/gripper_width', Float64, queue_size=10)
            self.gripper_is_grasped_pub = rospy.Publisher(f'/{robot_name}/gripper_is_grasped', Bool, queue_size=10)
            self.gripper_max_width_pub = rospy.Publisher(f'/{robot_name}/gripper_max_width', Float64, queue_size=10)
            rospy.loginfo(f"Franka状态读取器初始化完成: {robot_name}")
        else:
            self.pose_pub = None
            self.joint_pub = None
            self.gripper_width_pub = None
            self.gripper_is_grasped_pub = None
            self.gripper_max_width_pub = None
    ...
    def read_robot_state(self):
        ...
        with self.lock:
            try:
                ...
                torques = None  # 力矩信息可能不可用
                
                # 读取夹爪状态 - 使用正确的属性名
                gripper_width = self.gripper.width  # 当前夹爪宽度
                gripper_is_grasped = self.gripper.is_grasped  # 是否抓取到物体
                gripper_max_width = self.gripper.max_width  # 最大夹爪宽度
                ...
                self.current_gripper_width = gripper_width
                self.current_gripper_is_grasped = gripper_is_grasped
                self.current_gripper_max_width = gripper_max_width
    ...
    def publish_robot_state(self):
        ...
        try:
            ...
            if self.current_gripper_width is not None:
                width_msg = Float64()
                width_msg.data = float(self.current_gripper_width)
                self.gripper_width_pub.publish(width_msg)
            
            if self.current_gripper_is_grasped is not None:
                grasped_msg = Bool()
                grasped_msg.data = bool(self.current_gripper_is_grasped)
                self.gripper_is_grasped_pub.publish(grasped_msg)
            
            if self.current_gripper_max_width is not None:
                max_width_msg = Float64()
                max_width_msg.data = float(self.current_gripper_max_width)
                self.gripper_max_width_pub.publish(max_width_msg)
                ...
                if self.read_robot_state():
                    ...
                    if seq % 60 == 0:  # 30Hz * 2 = 60
                        if self.current_pose:
                            ...
                            if self.current_gripper_width is not None:
                                gripper_info = f", 夹爪: 宽度={self.current_gripper_width:.3f}m"
                                if self.current_gripper_is_grasped is not None:
                                    grasped_status = "已抓取" if self.current_gripper_is_grasped else "未抓取"
                                    gripper_info += f", 状态={grasped_status}"
                                if self.current_gripper_max_width is not None:
                                    gripper_info += f", 最大宽度={self.current_gripper_max_width:.3f}m"
                            
                            if self.enable_ros_publishing:
                                rospy.loginfo(f"{self.robot_name} - 位置: [{pos[0]:.3f}, {pos[1]:.3f}, {pos[2]:.3f}]{joint_info}{gripper_info}")
                            else:
                                print(f"{self.robot_name} - 位置: [{pos[0]:.3f}, {pos[1]:.3f}, {pos[2]:.3f}]{joint_info}{gripper_info}")
    ...
    def get_current_gripper_state(self):
        ...
        return {
            ...
            'is_grasped': self.current_gripper_is_grasped,
            'max_width': self.current_gripper_max_width,
            'timestamp': self.last_timestamp
        }
